...
1. Create the npm scope `@bedrockcompliance` (via `npm org create bedrockcompliance` or the web UI).
2. Add a repository secret `NPM_TOKEN` — an automation token scoped to `@bedrockcompliance`, 2FA auth-only.
